...
import com.example.smmousavi.maktab_hw82_remindemelater.mvc.controller.activities.UserLoginPagerActivity;
...
public class TasksListFragment extends Fragment {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    mLoggedInUserId = UserList.getInstance(getActivity()).getLoggedInUserId();
  ...
  public void updateListUI() {
    ...
    switch (mTaskListId) {
      case 0:
        mTasksList = taskList.getAllTasks(mLoggedInUserId);
        break;
      case 1:
        mTasksList = taskList.getUndoneTasks(mLoggedInUserId);
        break;
      case 2:
        mTasksList = taskList.getDoneTasks(mLoggedInUserId);
        break;
    }
  ...
  private class TaskViewHolder extends RecyclerView.ViewHolder {
    ...
    public TaskViewHolder(final View itemView) {
      ...
      itemView.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View view) {
          Task task = TaskList.getInstance(getActivity()).getTask(mClikedTask.getTaskId(), mClikedTask.getUserId());
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      ...
      case R.id.menu_item_clear_list:
        clearTasksList();
        return true;
      case R.id.menut_item_logout:
        showAlertDialog(R.string.log_out_cofirm, new DialogInterface.OnClickListener() {
          @Override
          public void onClick(DialogInterface dialogInterface, int i) {
            Intent intent = UserLoginPagerActivity.newIntent(getActivity());
            startActivity(intent);
  ...
  public void updateSubtitle() {
    ...
    User user = UserList.getInstance(getActivity()).getUser(mLoggedInUserId);
    String subtitle = getString(R.string.toolbar_subtitle_login_info, user.getName());
    if (!mSubtitleVisiable)
      subtitle = null;
  ...
  private void clearTasksList() {
    switch (mTaskListId) {
      case 0:
        if (TaskList.getInstance(getActivity()).getAllTasks(mLoggedInUserId).size() == 0)
          ...
          showAlertDialog(R.string.all_tasks_delete_confirm_alert, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
              TaskList.getInstance(getActivity()).clearAllTasks(mLoggedInUserId);
        ...
        break;
      case 1:
        if (TaskList.getInstance(getActivity()).getUndoneTasks(mLoggedInUserId).size() == 0)
          ...
          showAlertDialog(R.string.undone_tasks_delete_confirm_alert, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
              TaskList.getInstance(getActivity()).clearUndoneTasks(mLoggedInUserId);
        ...
        break;
      case 2:
        if (TaskList.getInstance(getActivity()).getDoneTasks(mLoggedInUserId).size() == 0)
          ...
          showAlertDialog(R.string.done_tasks_delete_confirm_alert, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
              TaskList.getInstance(getActivity()).clearDoneTasks(mLoggedInUserId);
